...
function listItem(bodyText, metaText, onDelete) {
...
async function render() {
  ...
  sectionsRoot.append(renderSection({
    ...
    items: data.userName
      ? [listItem(`びくたんはあなたを「${data.userName}」と呼びます`, "", () =>
        deleteItem("characterAnswer", "user_address", "呼び名"))]
      : []
  }));
  ...
  sectionsRoot.append(renderSection({
    ...
    items: data.learnedWords.map((word, index) =>
      listItem(word.text, word.date, () => deleteItem("learnedWord", index, "この言葉")))
  }));
  ...
  sectionsRoot.append(renderSection({
    ...
    items: data.sharedMemories.map((memory, index) =>
      listItem(memory.text, memory.date, () => deleteItem("sharedMemory", index, "この思い出")))
  }));
    ...
    items: [
      ...data.characterAnswers.map((answer) =>
        listItem(`${answer.question} → ${answer.answer}`, "", () =>
          deleteItem("characterAnswer", answer.id, "この回答"))),
      ...data.growthAnswers.map((answer) =>
        listItem(`${answer.question} → ${answer.answer}`, "", () =>
          deleteItem("growthAnswer", answer.id, "この回答")))
    ]
  }));
  ...
  sectionsRoot.append(renderSection({
    ...
    items: data.diaries.map((diary) =>
      listItem(diary.preview, diary.date, () => deleteItem("diary", diary.date, `${diary.date}の日記`)))
  }));
  ...
  sectionsRoot.append(renderSection({
    ...
    items: data.savedLinks.map((link) =>
      listItem(link.title, link.source, () => deleteItem("savedLink", link.url, "この記事")))
  }));
  ...
  sectionsRoot.append(renderSection({
    ...
    items: [listItem(
      ...
      "",
      () => clearCategory("history", "会話・セリフの履歴")
    )]
  }));
