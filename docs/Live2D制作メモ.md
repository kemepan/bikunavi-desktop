...
| f09 | ウインク | 顔変形が崩れるためアプリでは一時停止。winkは左目をOpen=1・Smile=1にする笑顔パラメータで代用 |
...
会話状態は `話す > 考える > 聞く > 待機` の優先順で判定し、約0.26秒かけて動きを混ぜる。先行読み上げが始まったら、回答生成の完了前でも考え中の目・ジングルから口パクと発話中の揺れへ移る。短い相づちは本回答と区別し、相づち後に回答待ちなら考え中へ戻す。

## 済んだこと（2026-07-20）

- f07×マスク・f08考え中・f09ウインクを追加（ユーザー作画）→ model3.jsonへマージ。f09は顔崩れが出たためアプリ連携だけ一時停止中。
