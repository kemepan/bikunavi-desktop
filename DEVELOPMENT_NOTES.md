...
- ウィンドウは `transparent: true`。`BrowserWindow.setOpacity()` は使わない方がよい。**吹き出しまで薄くなって読めなくなる**ため
- キャラクターだけ薄くするなら、`#stage`（canvas）へCSSの `opacity` を掛ける。吹き出しは別要素なので影響しない
- 「じゃま」には**クリックを遮る**意味も含まれる可能性がある。見た目だけ薄くしても、後ろのウィンドウは押せないまま。透けている間は `setIgnoreMouseEvents(true, { forward: true })` でクリックを通す案があるが、そうするとドラッグ移動もできなくなるので、戻す導線（トレイ、ホットキー）が要る
- 自動で薄くする場合の復帰条件は、既存の `isHovered` / `chatActive` / `isSpeaking` を流用できる。`resumeAmbientState()` の判定と同じ並び
