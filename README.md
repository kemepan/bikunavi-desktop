...
- 会話履歴の簡易表示
- 「お願いごと」— 話しかけると、登録しておいた決まった仕事を実行（確認の選択肢を出してから実行し、記録を残す。設定した人にだけ現れる。詳細は [`docs/お願いごとの設定.md`](docs/お願いごとの設定.md)）
- キャラクター本体の右クリックで、その場にメニューを表示
- 音楽が鳴っている時と、動画や通話の音が鳴っている時を見分けて反応を変える（ブラウザの音の扱いは設定で選べる）
- 会話の内容から言葉や好みを拾って覚える（覚えたものはデータ管理画面で確認・削除できる）
- 使い始めてからの日数を、節目の日に話す
...
- 会話AI（いずれか1つ。なくても定型セリフ・占い・ポモドーロ等は動きます）
  ...
  - **Claude API** — トレイメニュー「会話AI」からAPIキー（sk-ant-…）を設定します。macOSの安全な保存機能（Keychain連動）で暗号化し、暗号化したデータを `state.json` に保存します。
...
- 本リポジトリのソースコード、Live2Dモデル、キャラクター「びくたん」の権利は制作者（びくに / bikunitan.online）が保持します。アプリ・モデルの再配布、モデルデータの抽出・流用、コードの転用は許可していません。詳細は [`LICENSE.md`](LICENSE.md) を確認してください。
