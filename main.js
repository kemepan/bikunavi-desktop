...
function makeBikutanWorkLine(force = false) {
  ...
  const candidateLines = [
    ...
    // 数を言うだけだと報告で終わる。中身に触れて、そこから話が続く形にする。
    // 掘り下げた話そのものは、AI生成の独り言側（relationshipMemory の指示）に
    // 任せる。ここは定型なので、続きを促すところまでにする。
    learnedWords.length
      ? `ことば帳の「${String(learnedWords[learnedWords.length - 1]?.text || "").slice(0, 16)}」を眺めて、` +
        "次に話すならどこからかな、と考えていました。"
      : "ことば帳はまだ真っ白なので、何か教えてもらえたら嬉しいです。",
    sharedMemories.length
      ? `思い出帳の「${String(sharedMemories[sharedMemories.length - 1]?.text || "").slice(0, 20)}」を読み返して、` +
        "あの時の続きが気になっていました。"
      : "思い出帳に、今日の余白を作っています。",
...
async function generateIdleLines() {
  ...
  idleLineGeneration = (async () => {
    ...
    const prompt = [
      ...
      "出力形式は必ず `種別|参照ID|セリフ` にしてください。通常セリフは `normal||セリフ`、ニュース系（AI・デザイン・その他）は `news|A3|セリフ` や `news|D3|セリフ`、生活ハック系は `life|L3|セリフ` のように、元にした見出しのIDを1〜2個入れてください。直前の行の続きを話す時だけ `cont||セリフ` にしてください。",
      [
        "出力例（この内容自体は使わず、形式だけ真似してください）:",
        "normal||ボーンの名前、毎回どう付けるか迷います。",
        "cont||結局、半年後の自分が読めるかどうかなんですよね。",
        "cont||英語と日本語が混ざった時がいちばん困ります。",
        "news|A3|新しいモデルが出たみたいですよ。実際に動かしてみたいです。",
        "normal||そろそろコーヒー、淹れ直しどきかもしれません。"
      ].join("\n"),
      ...
      relationshipMemory
        ? [
          "20個のうち最大2個だけ、ことば帳や思い出帳の内容に触れて構いません。毎回同じ記憶を使わないでください。",
          // 「覚えています」で終わると、報告になって話が閉じる。
          // 覚えた言葉そのものを一段掘り下げて、中身のある話にしてもらう。
          "その時は「覚えています」「使ってみたいです」で終わらせず、**その言葉から一歩踏み込んだ話**にしてください。",
          "踏み込む方向の例: 作り方や淹れ方のコツ、種類や違い、合わせると良いもの、由来、季節や時間帯との相性、自分ならどうするか。",
          "例えば「カフェラテ」なら、覚えている報告ではなく、ミルクの温度やエスプレッソとの比率、浅煎りと深煎りの違い、といった具体の話にしてください。",
          // 作り話を防ぐ。ここを外すと、実在しない店や商品を言い出す。
          "ただし、店名・商品名・人名は、広く知られていて確実なものだけにしてください。少しでも自信がなければ出さず、一般的な話に留めてください。知らない事実は補わないでください。"
        ].join("\n")
        : "",
      recentConversationContext
      ...
      recentLinesForPrompt
        ? "以下は直近に話した内容です。同じ文だけでなく、言い換え、同じニュース、同じ助言、同じ質問、同じオチも避け、ここに出ていない話題を作ってください。これは過去に話した内容との重複を避ける指示です。続きもの（cont）の中で同じ話題を掘り下げるのは問題ありません。"
        : "",
